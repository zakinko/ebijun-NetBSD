...
class _Done(Exception):
    """Checks are in; skip the interactive path's teardown steps."""
...
# --- running the checks inside the image ---------------------------------
#
# Driving a console by typing at it works on a port QEMU can accelerate and
# does not work on one it interprets.  The virt68k Goldfish TTY threw away
# whatever was typed in the window after getty printed its prompt, echoed
# nothing back to say so, and left login collecting the retries as
# usernames until it timed out.
#
# So for those ports nothing is typed at all.  The checks are compiled into
# a shell script, installed in the image, and run at boot; the driver only
# reads.  Matching stays here in Python -- the script frames each command's
# output with markers and this side applies the patterns -- because the
# check files are written in Python regex and translating them to whatever
# grep the image has would quietly change what they mean.

BEGIN_RE = re.compile(r"CI-BEGIN (\d+)")
END_RE = re.compile(r"CI-END (\d+) rc=(-?\d+)")
DONE_RE = re.compile(r"CI-DONE-ALL")


def emit_script(steps):
    """Compile the checks into an sh script to run inside the image."""
    out = [
        "#!/bin/sh",
        "# Generated by ci/image/boot-verify.py --emit-script.  Runs at boot,",
        "# frames each check's output so the driver can read the results off",
        "# the console without typing anything back.",
        "exec 2>&1",
    ...
    for step in steps:
        ...
        if verb in ("expect", "absent"):
            ...
            # wait and send are console-driving verbs; they have no meaning
            # when nothing is being driven.
            continue
        out += [
            f"printf 'CI-BEGIN %s\\n' {lineno}",
            f"{cmd}",
            f"printf 'CI-END %s rc=%s\\n' {lineno} $?",
            "",
        ]
    out += [
        "printf 'CI-DONE-ALL\\n'",
...
def read_results(child, steps, timeout, console_timeout):
    """Watch the console for the framed results and check them."""
    wanted = {
        s[1]: s
        for s in steps
        if s[0] in ("expect", "absent", "run")
    }
    seen = {}
    log(f"reading results for {len(wanted)} checks off the console")

    deadline = time.time() + timeout
    while time.time() < deadline:
        idx = child.expect(
            [BEGIN_RE, DONE_RE, pexpect.TIMEOUT, pexpect.EOF],
            timeout=min(console_timeout, max(30, deadline - time.time())),
        )
        if idx == 1:
            break
        if idx == 2:
            raise Failure(
                "the image stopped producing output; "
                "the last of the console is in the log"
            )
        if idx == 3:
            raise Failure("QEMU exited before the checks finished")

        lineno = int(child.match.group(1))
        try:
            child.expect(END_RE, timeout=console_timeout)
        except pexpect.TIMEOUT:
            raise Failure(f"check on line {lineno} never finished")
        rc = int(child.match.group(2))
        body = "\n".join(
            l for l in child.before.splitlines() if "CI-BEGIN" not in l
        ).strip()
        seen[lineno] = (body, rc)
        log(f"got line {lineno} (rc={rc})")

    failures = []
    for lineno, step in sorted(wanted.items()):
        if lineno not in seen:
            failures.append(f"line {lineno}: never ran inside the image")
            continue
        body, rc = seen[lineno]
        verb = step[0]
        if verb == "run":
            if rc != 0:
                failures.append(f"line {lineno}: `{step[2]}` exited {rc}\n{body}")
        else:
            pattern, cmd = step[2], step[3]
            hit = re.search(pattern, body, re.MULTILINE) is not None
            if verb == "expect" and not hit:
                failures.append(
                    f"line {lineno}: `{cmd}` output did not match /{pattern}/\n{body}"
                )
            elif verb == "absent" and hit:
                failures.append(
                    f"line {lineno}: `{cmd}` output unexpectedly matched "
                    f"/{pattern}/\n{body}"
                )
    return failures
...
def main():
    ...
    ap.add_argument("--emit-script", action="store_true",
                    help="print the checks as an sh script to run inside the "
                         "image, and exit")
    ap.add_argument("--in-image", action="store_true",
                    help="do not drive the console; the image runs the checks "
                         "at boot and this only reads the results")
    ...
    if args.emit_script:
        sys.stdout.write(emit_script(steps))
        return 0
    ...
    with open(args.console_log, "w", encoding="utf-8") as console:
        ...
        try:
            if args.in_image:
                failures = read_results(
                    child, steps, args.boot_timeout, args.cmd_timeout
                )
                log(f"image finished in {time.time() - started:.0f}s")
                raise _Done
